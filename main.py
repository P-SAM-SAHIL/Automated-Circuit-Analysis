...
# ------------------------------------------------------------
# Initialize LLM client (used later by Part 2+)
# ------------------------------------------------------------

client = RobustLLMClient(
    base_url=GWDG_BASE_URL,
    api_key=""  # keep empty or load from env
)
...
# --- ADDED HELPER FUNCTION (Fixes KeyError) ---
...
for behavior in target_behaviors:
    ...
    advanced_interp = AdvancedAutomatedInterpreter(
        ...
        pair_data_induction=dynamic_pair_data,  # <-- injected here
...
with open("multi_task_discovery_report.md", "w") as f:
    f.write(full_report)

print("\n✓ Full multi-task exploration complete.")
'''
